...
public class Interceptors {
    ...
    public static void setupCameraTransformAfterHurtCameraEffect(
        float partialTicks
    ) {
        //if(1+1==2) return;
    ...
    private static PlayerWeaponInstance getPlayerWeaponInstance() {
        ...
        if (item != null && item instanceof Weapon) {
            ...
            ClientModContext context =
                (ClientModContext) weapon.getModContext();
    ...
    public static boolean setupViewBobbing(float partialTicks) {
    	...
    	*/
        ...
        if (
            ClientModContext.getContext() != null &&
            ClientModContext.getContext().getMainHeldWeapon() != null
        ) {
        ...
        GlStateManager.rotate(
            (float) ClientValueRepo.jumpingSpring.getLerpedPosition(),
            1,
            0,
            0
        );
        ...
        */
        ...
        if (pwi == null || !pwi.isAimed()) {
            ...
            GlStateManager.translate(
                MathHelper.sin(f1 * (float) Math.PI * speed) * f2 * 0.5F,
                -Math.abs(MathHelper.cos(f1 * (float) Math.PI) * f2) * 0.5f,
                0.0F
            );
            GlStateManager.rotate(
                MathHelper.sin(f1 * (float) Math.PI * speed) *
                    f2 *
                    3.0F *
                    sMult,
                0.0F,
            ...
            GlStateManager.rotate(
                Math.abs(
                    MathHelper.cos((f1 * (float) Math.PI - 0.2F) * speed) * f2
                ) * 5.0F,
                1.0F,
                0.0F,
                0.0F
            ...
            */
            ...
            float f3 =
                ...
                (entityplayer.cameraPitch - entityplayer.prevCameraPitch) *
                    partialTicks;
            GlStateManager.translate(
                MathHelper.sin(f1 * (float) Math.PI) * f2 * 0.2F,
                -Math.abs(MathHelper.cos(f1 * (float) Math.PI) * f2) * 0.2f,
                0.0F
            );
            GlStateManager.rotate(
                MathHelper.sin(f1 * (float) Math.PI) * f2 * 3.0F,
                0.0F,
                0.0F,
                1.0F
            );
            GlStateManager.rotate(
                Math.abs(MathHelper.cos(f1 * (float) Math.PI - 0.2F) * f2) *
                    5.0F,
                1.0F,
                0.0F,
                0.0F
...

        {
            SpreadableExposure spreadableExposure =
                CompatibleExposureCapability.getExposure(
                    entityplayer,
                    SpreadableExposure.class
                );
            ...
            if (spreadableExposure != null) {
                ...
                f2 = f2 * f2;
                GlStateManager.rotate(
                    ((float) spreadableExposure.getTickCount() + partialTicks) *
                        speed,
                    0.0F,
                    1.0F,
                    1.0F
        ...
        if (
            ...
            ModernConfigManager.enableScreenShaders
        ) {
    ...
    public static final ScreenshakingManager SCREENSHAKING_MANAGER =
        new ScreenshakingManager();
    ...
    public static void checkLayerRenderersHooks() {
        ...
        layerRendererField = ReflectionHelper.findField(
            RenderLivingBase.class,
            "layerRenderers",
            "field_177097_h"
        );
        translateItemField = ReflectionHelper.findMethod(
            LayerHeldItem.class,
            "translateToHand",
            "func_191361_a",
            EnumHandSide.class
    ...
    public static void positionItemSide(
        RenderLivingBase<?> livingEntityRenderer,
        ...
        if (entity instanceof EntityPlayer) {
            ((ModelBiped) livingEntityRenderer.getMainModel()).postRenderArm(
                0.0625F,
                handSide
            );
        } else {
    ...
    public static void turn(
    	...
    	*/
        ...
        if (
            ...
            weaponInstance.getScope() != null &&
            weaponInstance.getScope().isOptical()
        ) {
        ...
        if (
            ...
            ClientEventHandler.freecamEnabled
        ) {
        ...
        player.rotationYaw = (float) ((double) player.rotationYaw +
            (double) yawDelta);

        player.rotationPitch = (float) ((double) player.rotationPitch -
            (double) pitchDelta * 0.15);
