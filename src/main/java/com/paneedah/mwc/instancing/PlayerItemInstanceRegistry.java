...
public final class PlayerItemInstanceRegistry {
     ...
     */
    private final Map<UUID, Map<Integer, PlayerItemInstance<?>>> registry = new ConcurrentHashMap<>();
    ...
    public PlayerItemInstance<?> getItemInstance(final EntityPlayer player, final int slot) {
        final Map<Integer, PlayerItemInstance<?>> slotInstances = registry.computeIfAbsent(player.getPersistentID(), uuid -> new ConcurrentHashMap<>());
...
//        log.debug("Slot {} contains {}", slot, result);
    ...
    /// Retrieves a cached [PlayerItemInstance] associated with the given [ItemStack] for rendering purposes.
    ///
    /// It will attempt to retrieve the instance from the cache or load a new one if not found.
    ...
    ///
    /// @param entityLiving The entity holding the [ItemStack]
    /// @param itemStack The [ItemStack] whose associated [PlayerItemInstance] is to be retrieved
    ///
    /// @return The cached [PlayerItemInstance], or a temporary [PlayerItemInstance] if not found
    @SideOnly(CLIENT)
...
//            slotInstances.put(extendedStateToMerge.getItemInventoryIndex(), extendedStateToMerge);
    ...
    public void update(final EntityPlayer player) {
        ...
        for (final Iterator<Map.Entry<Integer, PlayerItemInstance<?>>> iterator = slotInstances.entrySet().iterator(); iterator.hasNext(); ) {
            final Map.Entry<Integer, PlayerItemInstance<?>> entry = iterator.next();
